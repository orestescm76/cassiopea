...
{
    public class Spotify : NotifyBase
    {
        ...
        private bool _avalilable;

        public bool SpotifyAvailable
        {
            get { return _avalilable; }
            set { _avalilable = value; OnPropertyChanged(); }
        }
        ...
        private void Start()
        {
            ...
            catch (Exception ex)
            {
                ...
                //MessageBox.Show(Kernel.GetText("error_internet")); TODO
            }
        }
        private async Task StartStreamMode()
        {
            ...
            catch (Exception e)
            {
                ...
                if (e.InnerException.Message == "invalid_grant")
                {
                    ...
                    //Kernel.ResetSpotifyLink(); //hace visible el link e invisible el import
        ...
        //this function is used to signal the logon has gone correctly.
        private async void SignalLogin()
        {
            SpotifyAvailable = true;
            if (!UserIsPremium())
            {
                Log.Instance.PrintMessage("User is not premium", MessageType.Warning);
                var box = MessageBoxManager.GetMessageBoxStandard("", Kernel.LocalTexts.GetString("noPremium"), ButtonEnum.Ok, Icon.Warning, WindowStartupLocation.CenterScreen);
            }
            else
                Log.Instance.PrintMessage("User is premium", MessageType.Info);
            await Task.Run(() =>
            {
        ...
        /// <summary>
        /// Inserts an album from a Spotify URI
        /// </summary>
        /// <param name="uri"></param>
        /// <returns>the processed album. can be null</returns>
        public AlbumData InsertAlbumFromURI(string uri)
        {
        ...
        public AlbumData ProcessAlbum(FullAlbum album, bool downloadCover = true)
        {
        ...
        private async void DownloadCover(FullAlbum album, string file_name)
        {
            using (HttpClient httpClient = new())
            {
                ...
                catch (HttpRequestException e)
                {
                    ...
                    //MessageBox.Show(Kernel.GetText("errorPortada"), "", MessageBoxButtons.OK, MessageBoxIcon.Error); TODO
                    file_name = "";
                }
                catch (Exception e)
                {
        ...
        public AlbumData ProcessAlbum(SimpleAlbum album)
        {
        ...
        public async void GetUserAlbums()
        {
            if (User is not null)
            {
                ...
                try
                {
                    ...
                    //.src.Forms.LoadBar loadBar = new src.Forms.LoadBar(limit, "Downloading albums");
