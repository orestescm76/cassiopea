...
namespace Cassiopeia
{
    public static class Utils
    {
        ...
        public static HashSet<AlbumData> GetAlbumsWithSongTitle(IEnumerable<AlbumData> albums, string song)
        {
            HashSet<AlbumData> found = new();
            for (int i = 0; i < albums.Count(); i++)
            {
                for (int j = 0; j < albums.ElementAt(i).Songs.Count; j++)
                {
                    Song s = albums.ElementAt(i).Songs[j];
